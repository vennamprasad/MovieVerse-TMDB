...
class GetSeriesRecommendationsUseCaseTest {

    private val repository: SeriesRepository = mockk()
    private lateinit var useCase: GetSeriesRecommendationsUseCase

    @BeforeEach
    fun setup() {
        useCase = GetSeriesRecommendationsUseCase(repository)
    }

    @Test
    fun `should fetch distinct series recommendations`() = runTest {
        ...
        coEvery { repository.getSeriesRecommendations(10, 1) } returns seriesList
        ...
        coVerify { repository.getSeriesRecommendations(10, 1) }
    }
}